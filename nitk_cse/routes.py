...
from nitk_cse.models import User, News, Research, ResearchJournal, ResearchConf, ResearchFaculty, Professor, CSFY, CSSY, \
    ISFY, ISSY, RDProjects, Consultancy, Facultyuser, rd_projects_ece, it_professor
from nitk_cse.forms import LoginForm, NewsForm, ResearchAreaForm, JournalForm, ConferenceForm, FacultyForm, \
    ProfessorForm, CSFYForm, CSSYForm, ISFYForm, ISSYForm, RDForm, ConsultancyForm, FacultyLoginForm
...
@app.route("/csedep")
def csedep():
    return render_template('Computersci.html')

@app.route("/itdep")
def itdep():
    return render_template('itdep.html')
...
@app.route("/itfaculty")
def itfaculty():
    profs = it_professor.query.filter_by(title="Professor")
    hod = it_professor.query.filter_by(title="Head of Department")
    asso = it_professor.query.filter_by(title="Associate Professor")
    assi = it_professor.query.filter_by(title="Assistant Professor")
    return render_template('itfaculty.html', profs=profs, hod=hod, asso=asso, assi=assi)
...
@app.route("/itstudent")
def itstudent():
     return render_template('itstudent.html')
...
@app.route('/itfy')
def itfy():
    return render_template('itfy.html')
@app.route('/itsy')
def itsy():
    return render_template('itsy.html')
...
@app.route("/facilities/guesthouse")
def guesthouse():
    return render_template('guesthouse.html', title='GUEST HOUSE')


@app.route("/facilities/campus")
def campus():
    return render_template('campus.html', title='Facilities')


@app.route('/rdproject')
def rdproject():
    projects = RDProjects.query.all()
    return render_template('rdlinks.html', projects=projects, title='Research & Consultancy')


@app.route('/rdproject/cse')
def rdprojectcse():
    projects = RDProjects.query.all()
    return render_template('rdproject.html', projects=projects, title='Research & Consultancy')


@app.route('/rdproject/ece')
def rdprojectece():
    projects = rd_projects_ece.query.all()
    return render_template('rdprojectece.html', projects=projects, title='Research & Consultancy')
...
@app.route('/placement/cse')
def placementcse():
    return render_template('cseplacements.html', title='CSEPlacement')


@app.route('/placement/it')
def placementit():
    return render_template('itplacements.html', title='ITPlacement')
...
@app.route('/ranking/ariia')
def rankingarria():
    return render_template('ariia.html', title='ari')


@app.route('/ranking/nirf')
def rankingnirf():
    return render_template('nirf.html', title='nirf')
...
def login():
    ...
    if form.validate_on_submit():
        # if form.email.data == 'admin@nitk_cse.edu.in' and form.password.data == 'admin':
...
def facultylogin():
    ...
    if form.validate_on_submit():
        ...
        #   flash(f'Login Unsuccesful!','danger')
        # return render_template('login.html',form=form)
        facultyuser = Facultyuser.query.filter_by(email=form.email.data).first()
        # print(facultyuser)
        if facultyuser:
            # print (user)
            if facultyuser.password == form.password.data:
                return render_template('loginfaculty.html', form=form)

        return '<h1>Invalid username or password</h1>'
        # return '<h1>' + form.email.data + ' ' + form.password.data + '</h1>'

    return render_template('facultylogin.html', form=form)
...
def add_consultancy():
    ...
    if form.validate_on_submit():
        r = Consultancy(title=form.title.data, guide=form.guide.data, agency=form.agency.data,
                        student=form.student.data, status=form.status.data, year=form.year.data)
        db.session.add(r)
        db.session.commit()
